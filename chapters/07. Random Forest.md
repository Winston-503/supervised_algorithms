...
Despite the fact that *bagging* can be applied with any type of algorithms, **bagging over decision trees** has become widespread. Since they are unstable and variable, a good result is obtained. In fact, **random forest** is **bagging over decision trees with random subspace method**.
...
- max_features - the number of features to draw from train set to train each base tree - `n/3` for regression and `sqrt(n)` for classification
...
Other than that, it **inherits all hyperparameters and the pros and cons of random forest**.